...
def dcnn_model(numChannels, dimension, numClasses):
    ...
    # 32 output filters, 1x1 convolution kernel
    # Relu activation function. Turns all negative pixel values to zero
    model.add(Convolution2D(32, (1, 1), activation='relu', input_shape=(dimension, dimension, numChannels))) # Add first layer
    model.add(Dropout(0.01))
    print(model.output)
    model.add(Convolution2D(16, (3, 3), activation='relu')) # edge detection?
    model.add(Dropout(0.01))
    print(model.output)
